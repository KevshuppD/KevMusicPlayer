...
- **Personalización de Interfaz y Temas Premium**:
  - **Temas de Colores**: Añadido soporte para 5 temas dinámicos: *Cyberpunk Púrpura* (estilo neón clásico), *Azul Petróleo* (minimalismo sofisticado a juego con el nuevo logo), *Obsidiana Oscuro* (negro puro AMOLED de alto contraste), *Turquesa* (vibrante turquesa neón y verde menta) y *Blanco y Negro* (Monochrome, tema claro premium con alto contraste que prioriza la legibilidad y adapta dinámicamente las barras de estado del sistema).
...
- **Ajustes de Audio Avanzados**: Ecualizador interactivo integrado con 10 presets predefinidos (*Plano*, *Clásica*, *Dance*, *Heavy Metal*, *Hip Hop*, *Jazz*, *Pop*, *Rock*, *Graves*, *Voz*) aplicados a 5 bandas de frecuencia. Los mandos de control circulares rotatorios (*Bass Boost* y *Virtualizador*) están limitados a valores seguros de 0% a 100% para evitar saltos inesperados en los bordes.
- **Traducción Automática de Letras**: Implementación de una opción de traducción automática que traduce dinámicamente las letras de las canciones al idioma actual de la aplicación si el idioma original es diferente.
- **Visualización de Portada en Álbumes**: Se añadió la visualización de la portada del álbum directamente al lado del título del álbum en las pantallas de detalle de biblioteca.
- **Barra Alfabética Lateral Inteligente**: La barra vertical de navegación alfabética rápida ahora se mantiene oculta para no entorpecer la lectura y se muestra/activa únicamente cuando se desliza el dedo por el borde derecho de la pantalla.
- **Gestos de Selección y Menú Contextual Simplificados**: Eliminación del botón de tres puntos en favor de una experiencia táctil fluida. Un toque prolongado sobre cualquier canción activa el menú de opciones (Reproducir a continuación, Añadir a cola, Añadir a playlist, Editar metadatos, Eliminar). El arrastre progresivo hacia abajo realiza la selección múltiple y el contenedor hace scroll automático al llegar al límite inferior de la pantalla. En este modo de selección múltiple se oculta el botón de reproducción rápida para evitar conflictos visuales.
- **Estabilización de Widgets Glance**: Solucionado el problema que impedía cargar los widgets de pantalla de inicio ("no se puede cargar el widget") debido a inconsistencias con los broadcasts del servicio de reproducción ExoPlayer en segundo plano.
...
## 7. Development Rules
- **Automatic Build & Test**: Every time a change or feature modification is completed, the developer/agent must execute `./gradlew installDebug` to compile, package, and deploy the application to the connected Android device or emulator for testing.
